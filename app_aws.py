...
app = Flask(__name__)
app.secret_key = 'blissful_abodes_secret_key_2024_production'
...
# SNS Topic ARN
SNS_TOPIC_ARN = f"arn:aws:sns:{REGION}:203918855127:hotel_booking_topic"
...
def admin_dashboard():
    ...
    try:
        ...
        logger.info(f"✓ Admin dashboard: {len(users)} users, {len(hotels)} hotels, {len(bookings)} bookings")
...
def admin_edit_hotel(hotel_id):
    ...
    try:
        ...
        return render_template('admin_edit_hotel.html', hotel=hotel, username=session['admin'])
    except ClientError as e:
